...
public class UserService {
    ...
    public static List<User> all() {
        List<User> users = (List<User>) DatabaseUtility.executeQuery_AutoMappingToModel("SELECT * FROM product ORDER BY id DESC", Product.class.getName());

        return users;
    }

    public static User find(int id) {
        User user = (User) DatabaseUtility.executeQuery_AutoMappingToModel("SELECT * FROM product WHERE id = " + id, Product.class.getName());
